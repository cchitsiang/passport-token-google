...
GoogleTokenStrategy.prototype.authenticate = function(req, options) {
  ...
  self._loadUserProfile(idToken || accessToken, function(err, profile) {
    ...
    if (self._passReqToCallback) {
      self._verify(req, idToken || accessToken, refreshToken, profile, verified);
    } else {
      self._verify(idToken || accessToken, refreshToken, profile, verified);
    }
  });
}
 ...
 *   - `displayName`
 *
 * @param {String} idTokenOrAccessToken
 * @param {Function} done
 * @api protected
 */
GoogleTokenStrategy.prototype.userProfile = function(idTokenOrAccessToken, done) {
  const decoded = jwt.decode(idTokenOrAccessToken, {complete: true});
  var profileUrl = decoded ? 'https://www.googleapis.com/oauth2/v3/tokeninfo?id_token='+idTokenOrAccessToken : 'https://www.googleapis.com/oauth2/v1/userinfo';
  this._oauth2.get(profileUrl, decoded ? null: idTokenOrAccessToken, function (err, body, res) {
 ...
 * @api private
 */
GoogleTokenStrategy.prototype._loadUserProfile = function(idTokenOrAccessToken, done) {
  var self = this;

  function loadIt() {
    return self.userProfile(idTokenOrAccessToken, done);
  }
  function skipIt() {
    return done(null);
  }
  ...
  if (typeof this._skipUserProfile == 'function' && this._skipUserProfile.length > 1) {
    // async
    this._skipUserProfile(idTokenOrAccessToken, function(err, skip) {
